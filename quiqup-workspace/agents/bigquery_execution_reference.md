...
| Break-Even | Meaning |
|------------|---------|
| **< 12 mo** | ✅ Good — recovers spend within a year |
| **12-24 mo** | 🟡 Acceptable — longer payback but viable |
| **> 24 mo** | ⚠️ Poor — unlikely to be profitable |
| **> 100 mo** | ❌ Catastrophic — effectively never breaks even |

---

## 📚 Source of Truth Tables — When to Use Each

This section defines which table is authoritative for each type of data. **Always use the designated source of truth** to ensure consistency across analyses.

### Primary Analysis Views (Start Here)

| Question | Source of Truth | Why |
|----------|-----------------|-----|
| **Marketing ROI, cohort LTV, break-even?** | `views.marketing_funnel_grandparent_monthly` | Pre-calculated cohort metrics with all joins already done |
| **Monthly marketing funnel (traffic only)?** | `views.quiqup_com_unique_visitors_monthly_2025_h2` | Traffic → leads → spend without cohort revenue |
| **Weekly CAC by marketing bucket?** | `views.cac` | Pre-computed with activation lag handling |

### Classification & Mapping Tables (Source of Truth)

| Data Type | Source of Truth Table | Use When | Key Columns |
|-----------|----------------------|----------|-------------|
| **Paid vs Organic channel** | `views.definition_paid_organic_channel` | Classifying any UTM as paid or organic | `utm_source`, `utm_medium` → `is_paid`, `channel_type` |
| **Lead source grouping** | `views.definition_lead_source` | Mapping raw `lead_source` to Inbound/Outbound/Old | `lead_source` → `source_group`, `is_website_source` |
| **UTM → Google Ads mapping** | `views.definition_utm_google_ads_mapping` | Linking Salesforce UTM to Google Ads campaign/ad group | `utm_campaign` → `google_ads_campaign`, `google_ads_ad_group` |
| **Campaign → marketing bucket** | `views.definition_campaign_marketing_bucket` | Grouping Google Ads campaigns by region (uae/ksa/marketplace) | `campaign_name` → `marketing_bucket` |
| **High-intent visitor definition** | `views.definition_high_intent_website_visitor` | Identifying visitors who viewed pricing, contact, demo pages | `path` patterns for high-intent pages |
| **Traffic channel mapping** | `views.definition_traffic_channel` | Classifying UTM source/medium to channel groups | `source_key`, `medium_key` → `channel_group` |

### Attribution Tables (Source of Truth)

| Data Type | Source of Truth Table | Use When | Critical Notes |
|-----------|----------------------|----------|----------------|
| **Account-level UTM attribution** | `views.account_utm_attribution` | Getting UTM for a Salesforce account (earliest-first across Lead→Opp→Account) | 11,695 accounts (31% coverage) |
| **Grandparent-level UTM attribution** | `views.grandparent_utm_attribution` | Getting UTM for a grandparent (1 grandparent = 1 acquisition) | Use for cohort analysis; handles SSUP exception |

### Revenue Tables (Source of Truth)

| Data Type | Source of Truth Table | Use When | Critical Notes |
|-----------|----------------------|----------|----------------|
| **Grandparent lifetime revenue** | `views.grandparent_revenue` | Getting total invoiced revenue per grandparent | **Invoice-to-parent deduplicated** — prevents 19x inflation |
| **Raw invoices** | `invoicer_current.invoices` | Drill-down to individual invoices | Must handle `invoice_to_parent` flag manually |

### Spend Tables (Source of Truth)

| Data Type | Source of Truth Table | Use When | Critical Notes |
|-----------|----------------------|----------|----------------|
| **Monthly Google Ads spend (aggregated)** | `views.google_ads_spend_monthly` | Monthly spend by campaign/ad group | Pre-aggregated, no dedup needed |
| **Daily Google Ads spend (raw)** | `google_ads_analytics.ads_CampaignBasicStats_8350869641` | Daily granularity or verification | Must filter dimension tables: `_DATA_DATE = _LATEST_DATE` |

### Grandparent/Account Tables (Source of Truth)

| Data Type | Source of Truth Table | Use When | Critical Notes |
|-----------|----------------------|----------|----------------|
| **ALL grandparents (activated or not)** | `views.grandparent_account_created_date_and_first_order_delivered` | Counting total acquisitions by cohort | Includes activation status via `first_delivered_order_date` |
| **True grandparent hierarchy** | `views.true_grandparent_account` | Resolving account → grandparent relationship | Handles SSUP parent (21,563 children treated as own grandparents) |
| **Account spine with daily snapshot** | `views.master_account_data_daily` | Account lifecycle, UTM, activation signals | Use `earliest_created_date` for cohorts |

### When to Query Underlying Tables

Only query underlying tables when you need:

1. **Drill-down beyond monthly granularity** → Use `salesforce_current.leads`, `google_ads_analytics.*`
2. **Individual lead/account details** → Use `salesforce_current.leads`, `salesforce_current.accounts`
3. **Raw invoice line items** → Use `invoicer_current.invoice_lines`
4. **Verification/reconciliation** → Cross-check views against raw tables

**⚠️ WARNING:** When querying raw Google Ads dimension tables (`ads_Campaign_*`, `ads_AdGroup_*`), you **MUST** filter `WHERE _DATA_DATE = _LATEST_DATE` to avoid cartesian products.
...
The invoicing system supports **hierarchical account structures** where child accounts can have invoices issued to their parent account. This is controlled by the `invoice_to_parent` flag.

**Key concept: Invoice-to-Parent**
```
Grandparent: NESSA GENERAL TRADING L.L.C (grandparent_salesforce_id)
    └── Parent: NESSA--Warehouse (parent_account_id: 1057)
            └── Child: Nysaa - Al Quoz (account_id: 470209)
                       └── invoice_to_parent = TRUE
                       └── Invoices issued to parent (account_id: 1057)
```
...
### Revenue Attribution Query Pattern

```sql
-- Get total invoiced revenue for accounts by campaign
WITH account_revenue AS (
  ...
  JOIN invoicer_current.invoices i
    ON i.account_id = CASE
         ...
         ELSE a.account_id
       END
  WHERE i.state NOT IN ('void', 'cancelled', 'draft')  -- Recognized revenue
    AND i.record_deleted = FALSE
    AND i.deleted_at IS NULL
    AND EXTRACT(YEAR FROM i.end_date) = 2025
  GROUP BY 1, 2, 3
)
SELECT
  sf.utm_campaign_c,
...
JOIN salesforce_current.accounts sf ON sf.id = ar.salesforce_id
WHERE sf.utm_campaign_c IS NOT NULL
```
...
## Marketing Spend Architecture

The marketing funnel data is organized in layers for both aggregated reporting and drill-down analysis:

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  🎯 PRIMARY REPORTING LAYER (Start Here)                                        │
│  ┌───────────────────────────────────┐  ┌─────────────────────────────────────┐ │
│  │ marketing_funnel_grandparent_     │  │ quiqup_com_unique_visitors_monthly_ │ │
│  │ monthly ⭐ MAIN VIEW              │  │ 2025_h2 (traffic + leads + spend)   │ │
│  │ • Cohort LTV, ROI, break-even     │  │ • Top-of-funnel metrics             │ │
│  │ • Paid vs organic revenue         │  │ • GA4 visitors, SF leads            │ │
│  └───────────────┬───────────────────┘  └──────────────────┬──────────────────┘ │
│                  │                                          │                   │
│  ┌───────────────▼──────────────────────────────────────────▼─────────────────┐ │
│  │                              cac (weekly CAC by bucket)                    │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  ⭐ SOURCE OF TRUTH: DEFINITION TABLES (Classification & Mapping)               │
│  ┌─────────────────────────────┐  ┌─────────────────────────────────────────┐   │
│  │ definition_paid_organic_    │  │ definition_utm_google_ads_mapping       │   │
│  │ channel ⭐                  │  │ • UTM campaign → Google Ads campaign    │   │
│  │ • Paid vs Organic rules     │  │ • UTM campaign → Ad group               │   │
│  │ • is_paid, channel_type     │  │ • campaign_type (search/pmax/meta)      │   │
│  └─────────────────────────────┘  └─────────────────────────────────────────┘   │
│  ┌─────────────────────────────┐  ┌─────────────────────────────────────────┐   │
│  │ definition_lead_source      │  │ definition_campaign_marketing_bucket    │   │
│  │ • lead_source → source_group│  │ • campaign_name → bucket (uae/ksa)      │   │
│  │ • is_website_source flag    │  └─────────────────────────────────────────┘   │
│  └─────────────────────────────┘  ┌─────────────────────────────────────────┐   │
│  ┌─────────────────────────────┐  │ definition_traffic_channel              │   │
│  │ definition_high_intent_     │  │ • source/medium → channel_group         │   │
│  │ website_visitor             │  │ • Paid Search, Paid Social, etc.        │   │
...
└─────────────────────────────────────────────────────────────────────────────────┘
                                         ...
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  ⭐ SOURCE OF TRUTH: ATTRIBUTION & REVENUE VIEWS                                │
│  ┌─────────────────────────────────────┐  ┌───────────────────────────────────┐ │
│  │ grandparent_utm_attribution         │  │ grandparent_revenue ⭐            │ │
│  │ • UTM per grandparent (deduped)     │  │ • Invoice-to-parent deduplicated  │ │
│  │ • acquisition_date for cohorts      │  │ • total_revenue_aed per GP        │ │
│  │ • Handles SSUP exception            │  │ • Prevents 19x revenue inflation  │ │
│  └─────────────────────────────────────┘  └───────────────────────────────────┘ │
│  ┌─────────────────────────────────────┐  ┌───────────────────────────────────┐ │
│  │ account_utm_attribution             │  │ google_ads_spend_monthly          │ │
│  │ • UTM per SF account                │  │ • Pre-aggregated spend by         │ │
│  │ • Earliest-first across Lead→Opp→   │  │   campaign/ad group               │ │
│  │   Account hierarchy                 │  │ • spend_aed, clicks, impressions  │ │
│  └─────────────────────────────────────┘  └───────────────────────────────────┘ │
│  ┌─────────────────────────────────────────────────────────────────────────────┐│
│  │ grandparent_account_created_date_and_first_order_delivered                  ││
│  │ • ALL grandparents (with or without UTM) • activation via first_order_date  ││
│  └─────────────────────────────────────────────────────────────────────────────┘│
│  ┌─────────────────────────────────────┐  ┌───────────────────────────────────┐ │
│  │ true_grandparent_account            │  │ master_account_data_daily         │ │
│  │ • Account → grandparent resolution  │  │ • Account spine + UTM + activation│ │
│  │ • Handles SSUP parent exception     │  │ • Use earliest_created_date       │ │
│  └─────────────────────────────────────┘  └───────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         │
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  RAW DATA LAYER (Drill-down only)                                               │
│  ┌─────────────────────┐ ┌─────────────────────┐ ┌─────────────────────────────┐│
│  │ salesforce_current  │ │ google_ads_analytics│ │ invoicer_current            ││
│  │ • leads             │ │ • ads_Campaign_*    │ │ • invoices                  ││
│  │ • accounts          │ │ • ads_AdGroup_*     │ │ • accounts                  ││
│  │ • opportunities     │ │ • ads_Campaign      │ │ • invoice_lines             ││
│  │ • tasks             │ │   BasicStats_*      │ │ ⚠️ Handle invoice_to_parent ││
│  └─────────────────────┘ └─────────────────────┘ └─────────────────────────────┘│
│  ┌─────────────────────┐ ┌─────────────────────┐                                │
│  │ ex_api_current      │ │ analytics_368564502 │  ⚠️ Raw Google Ads: MUST      │
│  │ • orders            │ │ • GA4 events_*      │  filter _DATA_DATE =          │
│  │ • business_accounts │ │ • Nested schema     │  _LATEST_DATE on dimension    │
│  │ • order_state_chgs  │ │                     │  tables!                      │
│  └─────────────────────┘ └─────────────────────┘                                │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| Need | Layer | Table |
|------|-------|-------|
| Marketing ROI, cohort analysis | Primary | `marketing_funnel_grandparent_monthly` |
| Traffic funnel metrics | Primary | `quiqup_com_unique_visitors_monthly_2025_h2` |
| Paid vs Organic classification | Definition | `definition_paid_organic_channel` |
| UTM → Google Ads mapping | Definition | `definition_utm_google_ads_mapping` |
| Grandparent revenue (LTV) | Attribution/Revenue | `grandparent_revenue` |
| Account-level UTM | Attribution/Revenue | `account_utm_attribution` |
| Individual leads/accounts | Raw | `salesforce_current.*` |
| Daily ad spend | Raw | `google_ads_analytics.ads_CampaignBasicStats_*` |

**Key patterns:**
- **Monthly funnel:** Use `views.quiqup_com_unique_visitors_monthly_2025_h2` for aggregated metrics + spend
- **Lead-level:** Join leads by `DATE_TRUNC(created_date, MONTH)` to get leads for a specific funnel month
- **Campaign breakdown:** Query `ads_CampaignBasicStats` directly with `definition_campaign_marketing_bucket` for bucket grouping
- **Attribution:** Use `leads.utm_campaign_c` → Google Ads mapping to attribute spend to specific leads

---

## Use Cases: Table Selection Guide

| Use Case | Recommended Tables | Why |
|----------|-------------------|-----|
| **⭐ Marketing ROI & cohort analysis** | `views.marketing_funnel_grandparent_monthly` | **Primary view** — cohort-based GP acquisition, activation, lifetime revenue, ROI |
| **Campaign spend totals** | `views.google_ads_spend_monthly` + `views.definition_campaign_marketing_bucket` | Pre-aggregated, no dedup needed |
| **Lead → spend attribution** | `salesforce_current.leads` + `views.definition_utm_google_ads_mapping` + `views.google_ads_spend_monthly` | UTM already mapped to campaigns |
| **Account → UTM attribution** | `views.account_utm_attribution` | Earliest-first attribution across Lead→Opp→Account hierarchy |
| **Grandparent → UTM attribution** | `views.grandparent_utm_attribution` | Earliest UTM per grandparent (deduped hierarchy) |
| **Grandparent → Revenue** | `views.grandparent_revenue` | Deduplicated invoice revenue per grandparent |
| **Campaign → Revenue (LTV)** | `views.grandparent_utm_attribution` + `views.grandparent_revenue` | Attribution + deduplicated revenue at grandparent level |
| **Monthly marketing funnel (traffic only)** | `views.quiqup_com_unique_visitors_monthly_2025_h2` | Traffic→leads→spend (no cohort revenue) |
| **CAC by cohort** | `views.cac` | Pre-computed weekly CAC by bucket |
| **Daily/raw spend** | `google_ads_analytics.ads_CampaignBasicStats_*` | When you need granularity (**must filter** `_DATA_DATE = _LATEST_DATE` on dimension tables) |

**Rule of thumb:** Start with `views.*` tables. Only go to `google_ads_analytics.*` raw tables when you need daily granularity or metrics not in the views.

---

## Paid Channel Identification: Two Contexts

**⚠️ IMPORTANT:** "Paid" is defined differently depending on the context:

| Context | Where | Field | Definition | Use Case |
|---------|-------|-------|------------|----------|
| **GA4 Traffic** | `quiqup_com_unique_visitors_monthly_2025_h2` | `paid_unique_visitors` | Has gclid/wbraid/gbraid OR channel_group='Paid*' OR utm_medium IN (cpc,ppc,paid,...) | Website traffic analysis, top-of-funnel |
| **Account Attribution** | `grandparent_utm_attribution` | `attributed_utm_medium = 'cpc'` | Only `utm_medium = 'cpc'` | ROI analysis, spend attribution |

**Why the difference?**
- **GA4 Traffic:** Broader definition to capture all paid traffic sources (Google, Meta, display ads, etc.)
- **Account Attribution:** Narrower definition (`cpc` only) because we need to match to Google Ads spend for ROI calculations

**When to use which:**
- Analyzing **website traffic** (visitors, sessions, page views) → use GA4 paid definition via H2 view
- Analyzing **ROI, CAC, or spend attribution** → use `attributed_utm_medium = 'cpc'` filter on grandparent attribution views
